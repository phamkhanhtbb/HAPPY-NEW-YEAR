...
// Thêm đoạn này để sửa lỗi mất tiếng
...
const WISH_MESSAGES = [
	...
	"Năm mới bình an",
	"Chúc mọi điều ước của cô ấy đều trở thành hiện thực ✨",
	"Chúc gia đình babe luôn bình an và hạnh phúc ❤️",
	"Chúc babe luôn khỏe mạnh và tràn đầy năng lượng tích cực 💪",
	"Chúc cổ vững bước trên con đường đã chọn  🚀",
	"Chúc cổ luôn mỉm cười và yêu đời mỗi ngày 😊",
	"Chúc cổ gặp nhiều may mắn và niềm vui (may mắn nhất là gặp đc toaii) 🎉",
];
...
function spawnWishMessage() {
	...
	inner.addEventListener('animationend', function() {
    	this.remove(); // Tự xóa chính nó khi bay xong
	});
}
...
// 	},
// };
// Thay thế toàn bộ đoạn soundManager cũ bằng đoạn này
const soundManager = {
    baseURL: "./audio/",
    // Không dùng ctx (AudioContext) nữa để tránh lỗi CORS khi chạy file trực tiếp
    sources: {
        lift: {
            volume: 1,
            playbackRateMin: 0.85,
    ...
    preload() {
        // Trả về Promise giả để code init() vẫn chạy bình thường mà không cần tải trước
        return Promise.resolve();
    },

    pauseAll() {
        // HTML5 Audio khó pause tất cả cùng lúc, tạm thời bỏ qua
    },
    ...
    playSound(type, scale = 1) {
        ...
        // Kiểm tra xem có đang tắt tiếng hoặc tua chậm không
        if (!canPlaySoundSelector() || simSpeed < 0.95) {
            return;
        }

        // Hạn chế âm thanh nổ nhỏ quá nhiều gây lag
        if (type === "burstSmall") {
            const now = Date.now();
            if (now - this._lastSmallBurstTime < 20) {
                return;
        ...
        // Chọn ngẫu nhiên 1 file trong danh sách
        const fileName = source.fileNames[Math.floor(Math.random() * source.fileNames.length)];
        
        // TẠO ÂM THANH MỚI BẰNG HTML5 AUDIO (Chạy được local)
        const audio = new Audio(this.baseURL + fileName);

        // Tính toán volume và tốc độ (giữ nguyên logic cũ)
        const initialVolume = source.volume;
        const initialPlaybackRate = MyMath.random(source.playbackRateMin, source.playbackRateMax);
        
        // Chỉnh volume
        audio.volume = initialVolume * scale;
        
        // Chỉnh tốc độ (playbackRate) - Lưu ý: một số trình duyệt cũ có thể không hỗ trợ đổi pitch
        audio.playbackRate = initialPlaybackRate * (2 - scale);
        // Quan trọng: Firefox/Chrome cần dòng này để đổi pitch mượt mà
        audio.preservesPitch = false; 

        // Phát nhạc (Bắt lỗi nếu chưa click màn hình)
        audio.play().catch(e => {
            // Lỗi này bình thường nếu người dùng chưa click vào trang web
        });

        // Tự động dọn dẹp biến audio khi phát xong (Garbage Collection của trình duyệt sẽ lo)
...
// ==========================================================
// CÀI ĐẶT MỚI: NHẠC NỀN & ĐẾM NGƯỢC
// ==========================================================

// 1. Tự động phát nhạc nền khi người dùng chạm vào màn hình
document.addEventListener('click', function() {
    const bgMusic = document.getElementById('bg-music');
    if (bgMusic && bgMusic.paused) {
        bgMusic.volume = 0.4; // Chỉnh âm lượng nhạc nền (0.0 đến 1.0)
        bgMusic.play().then(() => {
            console.log("Đang phát nhạc nền...");
        }).catch(e => {
            console.log("Chưa tìm thấy file bg-music.mp3 trong thư mục audio");
        });
    }
}, { once: true }); // Chỉ chạy lệnh này 1 lần đầu tiên
